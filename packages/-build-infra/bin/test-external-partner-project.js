...
function generatePackageConnectionCommand(useYarn, shouldLink = true) {
  const packageManager = useYarn ? 'yarn' : 'npm';
  const command = shouldLink ? `${packageManager} link` : `${packageManager} unlink`;
  const packageConnectionCommand = `
cd ${projectRoot};
${shouldLink ? (useYarn ? 'yarn install --force;' : 'npm install;') : ''}
cd ./packages/-build-infra;
${command};
cd ../adapter;
${command} @ember-data/-build-infra;
${command};
cd ../store;
${command} @ember-data/-build-infra;
${command} @ember-data/adapter;
${command};
cd ../serializer;
${command} @ember-data/-build-infra;
${command} @ember-data/store;
${command};
cd ../model;
${command} @ember-data/-build-infra;
${command} @ember-data/store;
...
${command} @ember-data/adapter;
${command} @ember-data/store;
${command} @ember-data/serializer;
${command} @ember-data/model;
${command};
`;
  return packageConnectionCommand;
}

function cleanup(isUsingYarn, ignoreErrors = false) {
  try {
    execExternal(`${isUsingYarn ? 'yarn' : 'npm'} unlink ember-data`);
  } catch (e) {
    !ignoreErrors ? console.error(e) : null;
  }
  try {
    execWithLog(generatePackageConnectionCommand(isUsingYarn, false));
  } catch (e) {
    !ignoreErrors ? console.error(e) : null;
  }
}

function exitWithCleanup(isUsingYarn, error) {
  cleanup(isUsingYarn);
  if (error) {
    throw error;
  }
...
try {
  // unlink previously linked packages if necessary
  cleanup(useYarn, true);
  execWithLog(generatePackageConnectionCommand(useYarn, true));
  execExternal(`${useYarn ? 'yarn install --force' : 'npm install'}`);
} catch (e) {
  debug(e);
  const err = new Error(
    `Unable to complete install of dependencies for external project ${externalProjectName}`
  );
  exitWithCleanup(useYarn, err);
}
...
if (skipSmokeTest && !commitTestPassed) {
  const err = new Error('Commit may result in a regression, but the smoke test was skipped.');
  exitWithCleanup(useYarn, err);
  ...
  const err = new Error(`Commit results in a regression in ${externalProjectName}`);
  exitWithCleanup(useYarn, err);
